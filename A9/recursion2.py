...
#  Description: More recursion exercises
...
def split53(nums):
    sum_one = 0
    sum_two = 0
    return split53Helper(0, nums, sum_one, sum_two)

def split53Helper(start, nums, sum_one, sum_two):
    if start >= len(nums):
        return sum_one == sum_two
    else:
        if nums[start] % 5 == 0:
            return split53Helper(start + 1, nums, sum_one + nums[start], sum_two)
        elif nums[start] % 3 == 0:
            return split53Helper(start + 1, nums, sum_one, sum_two + nums[start])
        else:
            return split53Helper(start + 1, nums, sum_one + nums[start], sum_two) or split53Helper(start + 1, nums, sum_one, sum_two + nums[start])
